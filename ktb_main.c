...
static ssize_t dynamic_eviction_read(struct kobject *kobj, struct kobj_attribute *attr,
						   char *buf)
{
...
static ssize_t dynamic_eviction_write(struct kobject *kobj, struct kobj_attribute *attr,
							const char *buf, size_t count)
{
	int ret;
		...
		uint64_t id)
{
	 ...
	 * ly you just loose one unique remote_id to it. More important point is the
...
int ktb_remotify_puts(void)
{
	 ...
	 * about whether our bloom filter was successfully sent across or not and 
	 * also without being concerned about whether we actually received any bloom
...
restartthread:
	...
	while(!kthread_should_stop())
	{
		...
		if((dynamic_eviction == 1) && (devict_count != 0))
		{
			evict_status = 2;
			/*
			 * if dynamic eviction count > system_unique_pages we would like to
			 * limit it to system_unique_pages. But is system_unique_pages >
			 * REDUCE_SYS_PAGES_BY then limit dynamic eviction count to
			 * REDUCE_SYS_PAGES_BY as we want to avoid too many evictions from
			 * keeping the eviction thread busy using up cpu.
			 */
			if(devict_count >= system_unique_pages)
			{
				//devict_count = system_unique_pages;
				(system_unique_pages >= REDUCE_SYS_PAGES_BY)?\
				(devict_count = REDUCE_SYS_PAGES_BY):(devict_count = system_unique_pages);
			}
		}
		else if(static_eviction && (system_unique_pages >= MAX_SYS_PAGES))
		{
			 ...
			 * - dynamic eviction being enabled but devict_count=0 and 
			 *   system_unique_pages going above MAX_SYS_PAGES 
			 * - dynamic eviction not set and 
			 *   system_unique_pages going above MAX_SYS_PAGES 
			 * static eviction kicks in. as static eviction is always set.
			 */
			evict_status = 1;
				...
				system_rscl_pcds)
		{
			...
			{ 
				...
				pr_info(" *** mtp | remote lookups succeeded: %llu|"
						" ktb_remotify_puts *** \n", succ_count);
			}
	...
	*/
	...
	if (kvm_tmem_bknd_enabled && use_cleancache)
	{
		...
		*/
		...
		ret = sysfs_create_group(kvm_tmem_bknd_devict, &devict_attr_group);
