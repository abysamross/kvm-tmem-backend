...
extern int show_msg_bloom_filter_add;
extern int debug_bloom_filter_add;
extern int show_msg_bloom_filter_check;
extern int debug_bloom_filter_check;
...
int bloom_filter_unref(struct bloom_filter *filter)
{
        int ret = 0;
	
        if(kref_put(&filter->kref, __bloom_filter_free))
                ret = 1;
	...
	list_for_each_entry(alg, &filter->alg_list, entry) {
		...
		if (ret < 0)
			goto exit_unlock;
                ...
                if(can_show(bloom_filter_add))
                        pr_info(" *** mtp | Bloom filter bit being set:"
                                " %u | bloom_filter_add*** \n", bit);
	...
	list_for_each_entry(alg, &filter->alg_list, entry) {
		...
		if (ret < 0)
			goto exit_unlock;
                ...
                if(can_show(bloom_filter_check))
                        pr_info(" *** mtp | Bloom filter bit being checked:"
                                " %u | bloom_filter_add*** \n", bit);
